        ...
        <div className="mx-auto mt-12 max-w-3xl divide-y divide-line border-y border-line">
          {faqs.map((item) => (
            <details key={item.q} className="group">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-4 text-[15px] text-ink marker:hidden">
                {item.q}
              ...
              <p className="max-w-prose pb-5 text-[14px] leading-[1.7] text-ink-soft">
                {item.a}
              </p>
            </details>
          ))}
